...
import com.example.bf_kotlin_client.models.ProductsCategoriesRequest
...
class MainActivityViewModel {
    ...
    private fun updateFieldCategories(data: String) {
        val response=Gson().fromJson(data, ProductsCategoriesRequest::class.java)
        fieldProductsCategories.set(response.productCategories.toString())
    }
    fun authorize() {
        appAuthApiWorker.auth(::updateFieldApiKey)
    }
    fun getAllCategories(){
        productCategoryApiWorker.getAll(::updateFieldCategories,GlobalVariables.instance.apiKey,
